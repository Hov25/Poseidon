...
def _load_asset_html(filename: str) -> str:
...
st.title("📊 Neptune 360 Tools")
tab_billing, tab_dials, tab_compare, tab_exp2csv, tab_miu = st.tabs(
    ["🧾 Billing File Audit", "📐 Dial Value Check", "🔍 File Comparator",
     "🔄 EXP -> CSV", "📟 MIU System Assessment"]
)
...
# --- TAB 2: Dial Value Check --------------------------------------------------

with tab_dials:
    st.subheader("Meter Size <-> Dial Value Check")
    st.caption(
        "Compares each account's actual dial count to the Neptune 360 standard "
        "expected for its meter size: **5/8\"-1\" -> 4 dials, 1-1/2\"-4\" -> 5 dials, "
        "6\"-12\" -> 6 dials, 16\"/20\" -> 7 dials.**"
    )

    if not st.session_state.get("billing_records"):
        st.info(
            "Parse files in the **🧾 Billing File Audit** tab first - this tab "
            "reuses those parsed records."
        )
    else:
        records = st.session_state["billing_records"]
        meta = st.session_state["billing_meta"]
        comparison = billing.compute_dial_comparison(records)

        from collections import Counter
        status_counts = Counter(r["dial_status"] for r in comparison)

        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Total Records", len(comparison))
        c2.metric("Match", status_counts.get("Match", 0))
        c3.metric("Mismatch", status_counts.get("Mismatch", 0))
        c4.metric("Unknown Size", status_counts.get("Unknown Size", 0))
        c5.metric("Missing Dials", status_counts.get("Missing Dials", 0))

        show_all = st.checkbox("Show all records (default shows only issues)", value=False)
        if show_all:
            display_records = comparison
        else:
            display_records = [r for r in comparison if r["dial_status"] != "Match"]
        ...
        if display_records:
            st.dataframe(
                [{
                    "Account #": r["account_number"], "Customer Name": r["customer_name"],
                    "Meter Number": r["meter_number"], "Collection ID": r["collection_id"],
                    "Meter Size": r["meter_size"],
                    "Expected Dials": r["expected_dials"] if r["expected_dials"] is not None else "--",
                    "Actual Dials": r["actual_dials"] if r["actual_dials"] is not None else "--",
                    "Status": r["dial_status"],
                } for r in display_records],
                use_container_width=True,
                height=400,
            )
        else:
            st.success("No mismatches, unknown sizes, or missing dials found.")

        with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tf:
            billing.export_dial_comparison_xlsx(records, meta, tf.name)
            tf.seek(0)
            data = Path(tf.name).read_bytes()

        st.download_button(
            "⬇ Download Dial Value Check Report",
            data=data,
            file_name="dial_value_check.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="dials_download",
        )


# --- TAB 3: File Comparator ----------------------------------------------------

with tab_compare:
    st.subheader("Import File Comparison")
    st.caption(
        "Compare two Neptune 360 import files (e.g. this month vs. last month) "
        "side by side on customer and account fields. Runs entirely in your "
        "browser - files never leave your machine."
    )
    components.html(_load_asset_html("comparator.html"), height=1000, scrolling=True)


# --- TAB 4: EXP -> CSV Converter ------------------------------------------------

with tab_exp2csv:
    st.subheader("Meter Reading Export (.exp) -> CSV Converter")
    st.caption(
        "Convert Neptune 360 .exp meter-reading export files to CSV with "
        "configurable output options. Runs entirely in your browser - files "
        "never leave your machine."
    )
    components.html(_load_asset_html("exp_to_csv.html"), height=1400, scrolling=True)


# --- TAB 5: MIU System Assessment ---------------------------------------------
